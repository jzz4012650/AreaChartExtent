...
export default class BarChartExtent extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.PADDING_SCALE = 1/5;
        this.dateFormat   = d3.time.format('%b %Y');
        this.xOrdinal     = d3.scale.ordinal().domain(this.props.data.map(d => d.date));
        this.xScale       = d3.scale.linear().domain([0, this.props.data.length - 1]);
        this.timeScale    = d3.time.scale().domain(d3.extent(this.props.data, d => this.dateFormat.parse(d.date)));
        this.yScale       = d3.scale.linear().domain([0, d3.max(this.props.data, d => d.count)]);
        this.xAxis        = d3.svg.axis().scale(this.timeScale).orient('bottom').ticks(d3.time.month, 6);
    ...
    doResize() {
        ...
        var range = [margin[3], width - margin[1]];

        this.xOrdinal.rangeBands(range, 0.2, 0);
        this.xScale.range(range);
        this.yScale.range([height - margin[2], margin[0]]);

        var rangeBand = this.xOrdinal.rangeBand();

        this.timeScale.range([range[0] + rangeBand / 2, range[1] - rangeBand / 2]);
    ...
    render() {
        var { width, height, extent } = this.state;
        var { margin, data } = this.props;

        var bars = width && height && data.map((d, i) => {
            var inExtent = extent.length && i >= extent[0] && i <= extent[1];
            var classNames = ["bar", inExtent ? " selected" : ""];
            return <rect className={classNames.join(' ')} key={i}
                        x={this.xOrdinal(d.date)}
                        y={this.yScale(d.count)}
                        width={this.xOrdinal.rangeBand()}
                        height={height - margin[2] - this.yScale(d.count)} />
        })

        return (
            <svg ref="svg" className="bar-chart-extent" width={this.state.width} height={this.state.height}>
                <g className="bar-axis" ref="axis" transform={'translate(0,' + (height - margin[2]) + ')'}></g>
                <g className="bar-chart">{bars}</g>
                <g className="bar-brush" ref="brush" transform={'translate(0,' + margin[0] + ')'}></g>
            </svg>
        );
    }
}

BarChartExtent.defaultProps = {
    margin: [20,20,40,20],
    data: [
        {date:"Jan 2000", count: 1394.46},
